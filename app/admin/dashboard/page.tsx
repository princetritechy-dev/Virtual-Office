...
type UserItem = {
  ...
  documents: {
    document_1: string;
    document_2: string;
    document_1_key?: string;
    document_2_key?: string;
  };
};
...
const handleViewDocument = async (documentKey: string) => {
  try {
    console.log("Opening S3 key:", documentKey);

    if (!documentKey) {
      alert("Document key missing.");
      return;
    }

    const res = await fetch(
      `/api/admin/view-document?key=${encodeURIComponent(documentKey)}`,
      {
        method: "GET",
      }
    );
                      ...
                      <div className="admin-links">
                        {user.documents.document_1 && (
                          <button
                            ...
                            onClick={() =>
                              handleViewDocument(user.documents.document_1_key || "")
                            }
                          >
                            View Document 1
                          </button>
                        ...
                        {user.documents.document_2 && (
                          <button
                            ...
                            className="admin-doc-btn"
                            onClick={() =>
                              handleViewDocument(user.documents.document_2_key || "")
                            }
                          >
                            View Document 2
                          </button>
